...
def withinclassrand(seed, plinkCluster, datafile):
    ...
    for cl in cluster_numbers:
        ...
        clusters.append([
                cluster_data.iloc[ind[0], 0],
                cluster_data.iloc[ind[0], 1],
                cl,
                cluster_data.iloc[ind[1], 0],
                cluster_data.iloc[ind[1], 1],
                ])
    clusters = np.array(clusters)
    
    # randomize
    rng = np.random.default_rng(seed)
    randidx = rng.permutation(clusters.shape[0])
    ...
    tmp2 = pid[pid.isin(p1[randidx[0:n]])].index.to_numpy()
